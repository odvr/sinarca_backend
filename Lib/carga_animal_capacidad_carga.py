      ...
      for i in consulta_bovinos:
          ...
          for i in consulta_bovinos_modulo:
              ...
              if estado_animal=="Muerto" or estado_animal=="Vendido":
                  session.execute(modelo_carga_animal_y_consumo_agua.delete().where(modelo_carga_animal_y_consumo_agua.c.id_bovino == id_bov))
                  session.commit()
              else:
                  pass
...
def capacidad_carga():
  try:
    ...
    for i in consulta_aforo:
        ...
        #el aforo determina cuantos gramos de materia seca produce un metro cuadrado de pasto en el predio
        aforo = i[1]
