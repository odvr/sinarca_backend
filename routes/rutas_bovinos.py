...
def inventario_prod_leche():
    ...
    try:
        itemsLeche = session.execute(modelo_leche.select()).fetchall()
    finally:
        session.close()
    return itemsLeche
...
def CrearProdLeche(fecha_primer_parto: date, id_bovino: str, fecha_inicial_ordeno: date, fecha_fin_ordeno: date,
...
def vacas_vacias():
    results = session.query(modelo_bovinos_inventario.c.estado, modelo_leche.c.datos_prenez). \
...
def vacas_prenadas():
    try:
        ...
        consulta_prenadas = connp.execute(
            select(join_tabla).where(modelo_leche.columns.datos_prenez == "Prenada")).all()
...
def animales_ceba():
    ...
    prop_ceba = condb.execute(select(modelo_bovinos_inventario).
                              where(modelo_bovinos_inventario.columns.proposito == "Ceba").filter_by(
        estado="Vivo")).rowcount
    # actualizacion de campos
    condb.execute(update(modelo_indicadores).
                  where(modelo_indicadores.columns.id_indicadores).
...
def animales_levante():
    ...
    prop_levante = condb.execute(select(modelo_bovinos_inventario).
                                 where(modelo_bovinos_inventario.columns.proposito == "Levante").filter_by(
        estado="Vivo")).rowcount
    # actualizacion de campos
    condb.execute(update(modelo_indicadores).
                  where(modelo_indicadores.columns.id_indicadores).
...
def animales_leche():
    ...
    prop_leche = condb.execute(select(modelo_bovinos_inventario).
                               where(modelo_bovinos_inventario.columns.proposito == "Leche").filter_by(
        estado="Vivo")).rowcount
    # actualizacion de campos
    condb.execute(update(modelo_indicadores).
                  where(modelo_indicadores.columns.id_indicadores).
...
async  def animales_en_ordeno():
    ...
    try:
        ...
        consulta_ordeno = conordeno.execute(select(join_tabla).where(modelo_leche.columns.ordeno == "Si")).all()
        #consulta_ordeno = select(join_tabla).where(modelo_leche.columns.ordeno == "Si").execute().fetchall()
...
def animales_edad_0_9():
    ...
    edades_0_9 = condb.execute(select(modelo_bovinos_inventario).
                               where(between(modelo_bovinos_inventario.columns.edad, 0, 9)).filter_by(
        estado="Vivo")).rowcount
    # actualizacion de campos
    condb.execute(update(modelo_indicadores).
                  where(modelo_indicadores.columns.id_indicadores).
...
def animales_edad_9_12():
    ...
    edades_9_12 = condb.execute(select(modelo_bovinos_inventario).
                                where(between(modelo_bovinos_inventario.columns.edad, 10, 12)).filter_by(
        estado="Vivo")).rowcount
    # actualizacion de campos
    condb.execute(update(modelo_indicadores).
                  where(modelo_indicadores.columns.id_indicadores).
...
def animales_edad_12_24():
    ...
    edades_12_24 = condb.execute(select(modelo_bovinos_inventario).
                                 where(between(modelo_bovinos_inventario.columns.edad, 13, 24)).filter_by(
        estado="Vivo")).rowcount
    # actualizacion de campos
    condb.execute(update(modelo_indicadores).
                  where(modelo_indicadores.columns.id_indicadores).
...
def animales_edad_24_36():
    ...
    edades_24_36 = condb.execute(select(modelo_bovinos_inventario).
                                 where(between(modelo_bovinos_inventario.columns.edad, 25, 36)).filter_by(
        estado="Vivo")).rowcount
    # actualizacion de campos
    condb.execute(update(modelo_indicadores).
                  where(modelo_indicadores.columns.id_indicadores).
...
def animales_edad_mayor_a_36():
    ...
    edades_36 = condb.execute(select(modelo_bovinos_inventario).
                              where(between(modelo_bovinos_inventario.columns.edad, 37, 500)).filter_by(
        estado="Vivo")).rowcount
    # actualizacion de campos
    condb.execute(update(modelo_indicadores).
                  where(modelo_indicadores.columns.id_indicadores).
...
def Animales_Optimo_Levante():
    ...
    join_tabla_levante = modelo_bovinos_inventario.join(modelo_levante,
                                                        modelo_bovinos_inventario.columns.id_bovino == modelo_levante.columns.id_bovino)
    # consulta a la tabla join de animales con estado optimo
    consulta_estado_levante = condb.execute(
        select(join_tabla_levante).where(modelo_levante.columns.estado_optimo_levante == "Estado Optimo")).all()
    # conversion a string y conteo de aquellos con estado vivo
...
def Animales_Optimo_Ceba():
    ...
    join_tabla_ceba = modelo_bovinos_inventario.join(modelo_ceba,
                                                     modelo_bovinos_inventario.columns.id_bovino == modelo_ceba.columns.id_bovino)
    # consulta a la tabla join de animales en ceba con estado optimo
    consulta_estado_ceba = condb.execute(
        select(join_tabla_ceba).where(modelo_ceba.columns.estado_optimo_ceba == "Estado Optimo")).all()
    # conversion a string y conteo de aquellos con estado vivo
